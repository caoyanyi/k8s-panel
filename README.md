...
- Service/Ingress 网络清单与 ConfigMap/Secret 最小配置摘要
...
Kubernetes 单类资源清单最多读取 5,000 项、20 页和 32 MiB 原始数据，Web 表格每页只渲染 100 行；事件仅在打开对应标签后读取，操作中心在页面不可见时暂停轮询。配置清单使用 Kubernetes Table 内容协商，只接收最小摘要且不回退读取完整对象；Secret 必须限定到单个命名空间。超过后端上限会停止读取并返回错误，避免大集群拖垮控制面。
