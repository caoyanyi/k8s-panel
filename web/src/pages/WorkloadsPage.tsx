...
export function WorkloadsPage() {
  ...
  const summary = useResource(
    (signal) => selectedClusterId ? api.get<ClusterSummary>(`/api/v1/clusters/${selectedClusterId}/summary`, signal) : Promise.resolve(null as unknown as ClusterSummary),
    [selectedClusterId],
  )
  ...
  return (
    <div className="page">
      <PageHeader
        title="工作负载"
        meta={selectedCluster ? `${selectedCluster.name} · ${summary.data?.version ?? selectedCluster.version ?? '-'}` : '选择一个集群'}
